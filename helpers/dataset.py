...
class DemoDataset(Dataset):
    ...
    def __init__(self, np_rng, expert_path, num_demos, max_ep_steps, wrap_absorb):
        ...
        for i, f in enumerate(sorted(Path(expert_path).glob('*.h5'))):
            ...
            logger.info(f"[DEMO DATASET] {"ep_len".ljust(20, '-')}{ep_len}")
            logger.info(f"[DEMO DATASET] {"ep_ret".ljust(20, '-')}{ep_ret}")
            ...
            terminal = not ep_len == max_ep_steps
            ...
            logger.info(f"[DEMO DATASET] {"subsample".ljust(15, '-')}{substr}")
            for k in tmp.keys():
                tmp[k] = tmp[k][indices]
